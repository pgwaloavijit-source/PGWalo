...
export const buildMonthlyInvoice = (
  ...
  return {
    ...
    organizationId: resident.organizationId || DEFAULT_ORGANIZATION_ID,
    invoiceNumber: `INV-${monthDate.getFullYear()}-${String(monthDate.getMonth() + 1).padStart(2, '0')}-${resident.id.slice(-6)}`,
    residentId: resident.id,
    ...
    roomNumber: resident.roomNumber,
    month,
    billingCycleStart: `${monthDate.getFullYear()}-${String(monthDate.getMonth() + 1).padStart(2, '0')}-01`,
    billingCycleEnd: new Date(monthDate.getFullYear(), monthDate.getMonth() + 1, 0).toISOString().split('T')[0],
    dueDate: dueDate.toISOString().split('T')[0],
    lines,
    amount,
    verifiedPaidAmount: 0,
    baseRent: rentPlan.monthlyRent,
    electricityCharges: 0,
    otherCharges: 0,
    lineItems: lines.map((line) => ({
      ...line,
      type: line.type === 'Previous Due' ? 'Previous Due' : 'Rent',
    })),
    subtotal: amount,
    previousDuesApplied: resident.previousDues || 0,
    advanceDeducted: 0,
    totalDue: amount,
    amountPaid: 0,
